...
- **智能转义**（marker 机制）：文本阶段在每个 Markdown 特殊字符（`\ * _ - + . > | $ # = [ ] ( ) ! ~ ` " '`）前插入占位符 `\a`(0x07)；Post-Render 阶段由一组 UnEscaper（IsItalicOrBold/IsBlockQuote/IsAtxHeader/IsSetextHeader/IsDivider/IsOrderedList/IsUnorderedList/IsImageOrLink/IsFencedCode/IsInlineCode/IsBackslash）按上下文判断该字符是否真的会被解析成 Markdown 语法——是则把占位符替换为 `\`，否则删除占位符。
...
- **扩展插件**：table（GFM 表格）、strikethrough（`~~`），v1 之后再做。
...
├── render_heading.mbt / render_emphasis.mbt / render_link.mbt / render_image.mbt
...
| P7 可选            | table（GFM）、strikethrough、Renderer 自定义钩子                                                | 独立子包 plugin/table 等               |
...
4. **URL 处理简化**：不实现完整 RFC3986 → 对齐 Go 版实际行为的子集（# 直返、data: 直返、\n\t 预编码、域名拼接、查询重编码、`%20` 替代 `+`），用 url_test.go 用例约束。
